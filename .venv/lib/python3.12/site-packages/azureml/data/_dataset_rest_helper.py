...
def _init_dataset(workspace, dataset_type, dataflow_json, properties, registration, dataset=None):
    ...
    try:
        rs_dataflow = _fetch_rslex_dataflow_yaml(workspace, fetch_id)
        ...
        rs_dataflow = None  # still want an attribute even if not fetched successfully
        raise RuntimeError('Failed to fetch RSLex YAML representation for the dataset, this dataset is no longer '
                           'supported, downgrade to previous version of the SDK.')

    definition = rs_dataflow
    if dataset is None:
