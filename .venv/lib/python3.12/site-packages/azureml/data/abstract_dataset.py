...
class AbstractDataset(object):
    ...
    def __init__(self):
        ...
        self._original_dataflow_json = None
    ...
    def get_by_name(workspace, name, version='latest', **kwargs):
    ...
    @staticmethod
    @track(_get_logger, activity_type=_PUBLIC_API)
    def get_by_id(workspace, id, **kwargs):
    ...
    def data_changed_time(self):
        ...
        """
        dataflow = self._dataflow.data_changed_time(column='last_modified_time')
        return dataflow.take(1)._to_pyrecords()[0]['last_modified_time']
    ...
    def _dataflow(self):
        ...
        # definition needs to be deserialized on first access
        if isinstance(self._definition, str):
            if not self._definition.strip().startswith('{'):
                try:
                    # payload is not json so try to load as EnginelessDataflow
                    self._definition = dataprep().EnginelessDataflow(self._definition,
                                                                     dataflow_json=self._original_dataflow_json)
                    _get_logger().info('Loaded dataset definition without dataprep engine.')
                except Exception as e:
                    msg = 'Failed to load dataset definition with azureml-dataprep=={}. (Engineless)'\
                        .format(dataprep().__version__)
                ...
                try:
                    # payload is json, older dataprep versions and customers that still use Dataflow with Engine
                    self._definition = dataprep().Dataflow.from_json(self._definition)
                    _get_logger().info('Loaded dataset definition with dataprep engine.')
                    # for backwards compatibility with older azureml-dataprep versions
                    # TODO: this should be removed after azureml-sdk with is released with new dependency
                    if not hasattr(self._definition, '_rs_dataflow_yaml'):
                        self._definition._rs_dataflow_yaml = None
                except Exception as e:
                    msg = 'Failed to load dataset definition with azureml-dataprep=={}'\
                        .format(dataprep().__version__)
                    _get_logger().error('{}. Exception: {}'.format(msg, e))
                    raise UserErrorException('{}. Please install the latest version with "pip install -U '
                                             'azureml-dataprep".'.format(msg))
    ...
    def _get_partition_key_values_with_common_path(self, partition_keys=None):
        ...
        try:
            ...
            success = True
        except dataprep().ExecutionError as e:
    ...
    def _get_partition_using_partition_key_values_common_path(self, partition_key_values_common_path, verbose=False):
        ...
        for k, v in partition_kv_dict.items():
            new_filter_condition = dataprep().col(k) == v
    ...
    def _get_partition_key_values(self, get_common_path=False, partition_keys=None):
        ...
        if not get_common_path:
            ...
            dataflow = dataflow.summarize(
                summary_columns=[
                    dataprep().SummaryColumnsValue(
                        column_id=_PATH_COLUMN_NAME,
                        summary_column_name=_COMMON_PATH_SUMMARY_COLUMN_NAME,
                        summary_function=dataprep().SummaryFunction.COMMONPATH)
                ],
    ...
    def _load(path: str, workspace, dataType=None):
        ...
        def _make_all_paths_absolute(mltable_yaml_dict, base_path):
            ...
            def format_path_pair(path_prop, path):
                # get absolute path from base_path + relative path
                if _is_local_path(path) and not path.startswith(_FILE_HANDLER):
                    ...
                    if _parse_path_format(base_path)[0] == _PathType.local:
                        path = _FILE_HANDLER + os.path.abspath(os.path.normpath(path))
            ...
            def format_paths(paths):
                return list(map(lambda path_dict:
                                tuple([path_dict,
                                      dict(map(lambda x: format_path_pair(*x), path_dict.items()))]), paths))

            if _PATHS_KEY in mltable_yaml_dict:
                path_pairs = format_paths(mltable_yaml_dict[_PATHS_KEY])
        ...
        def _append_workspace_to_stream_info_conversion(dataflow_yaml):
            if 'transformations' in dataflow_yaml:
                for transformation in dataflow_yaml['transformations']:
                    if isinstance(transformation, dict):
                        for name, info in transformation.items():
                            if name == 'convert_column_types':
                                for col_info in info:
                                    # 'stream_info' with no workspace info
                                    if 'stream_info' == col_info['column_type']:
            ...
            return dataflow_yaml

        import yaml

        with open(local_yaml_path, "r") as stream:
            try:
                mltable_yml_dict = yaml.safe_load(stream)
                if mltable_yml_dict is None or len(mltable_yml_dict) == 0:
                    _get_logger().error("mltable_yaml can't be empty")
                    raise UserErrorException("mltable_yaml can't be empty")

                if type(mltable_yml_dict) is not dict:
                    _get_logger().error("mltable_yaml is invalid. It needs to be type of dictiory")
                    raise UserErrorException("mltable_yaml is invalid. It needs to be type of dictionary")

                mltable_yml_dict = _make_all_paths_absolute(mltable_yml_dict, path)
                mltable_yml_dict = _append_workspace_to_stream_info_conversion(mltable_yml_dict)
                dataflow = dataprep().EnginelessDataflow(mltable_yml_dict)

                from azureml.data.tabular_dataset import TabularDataset
                from azureml.data.file_dataset import FileDataset
                if _is_tabular(mltable_yml_dict):
    ...
    @classmethod
    @track(_get_logger)
    def _create(cls, definition, properties=None, registration=None, telemetry_info=None, dataflow_json=None):
        ...
        dataset._original_dataflow_json = dataflow_json
        return dataset
    ...
    def _ensure_saved_internal(self, workspace):
        if not self._registration or not self._registration.saved_id:
            ...
            self._original_dataflow_json = saved_dataset._original_dataflow_json
...
class _PartitionKeyValueCommonPath():
    ...
    def __repr__(self):
        return f"KeyValues: {self.key_values}, CommonPath: {self.common_path}"
