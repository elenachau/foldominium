...
class TabularDatasetFactory:
    ...
    def _from_parquet_files(path, validate=True, include_path=False, set_column_types=None, partition_format=None):
        ...
        dataflow = _transform_and_validate(
            ...
            validate=validate,
            infer_column_types=_is_inference_required(set_column_types, validate))
    ...
    def from_sql_query(query, validate=True, set_column_types=None, query_timeout=30):
        ...
        if validate or _is_inference_required(set_column_types, validate):
    ...
    def register_pandas_dataframe(dataframe,
        ...
        :param make_target_path_unique: Optional, indicates if unique subfolder should be created in the target.
            Defaults to be True.
        :type show_progress: bool
        ...
        if make_target_path_unique:
            guid = uuid4()
            relative_path_with_guid = '%s/%s/' % (relative_path, guid)
        else:
            relative_path_with_guid = '%s/' % relative_path
            console(f'Using target path without unique subfolder: {relative_path_with_guid}.')
...
class FileDatasetFactory:
    ...
    def from_files(path, validate=True, partition_format=None, is_file=False):
        ...
        _trace_dataset_creation(path)
        dataflow = None
        if is_file and validate:
            warnings.warn("No validation will be performed because `is_file` is set to True.")

        dataflow = dataprep().EnginelessDataflow.from_paths(path, is_file)

        if partition_format:
            dataflow = _transform_and_validate(dataflow, partition_format, validate)
...
def _is_inference_required(set_column_types, validate):
    if set_column_types is None:
        return False
    try:
        for column, data_type in set_column_types.items():
            if isinstance(data_type._type_conversion, dataprep().DateTimeConverter) \
               and data_type._type_conversion.formats is None:
                if not validate:
                    warnings.warn(f'set_column_types for column {column} does not have date formats specified. '
                                  'Turning inference on to fill in the formats. This will require data access.')
                return True
    except Exception:
...
def _transform_and_validate(dataflow, partition_format, validate, infer_column_types=False):
    if partition_format:
    ...
    if validate:
        _validate_has_data(dataflow, 'Failed to validate the data.'
                                     'If data is inaccessible, please set validate to False.')
    elif infer_column_types:
        _validate_has_data(dataflow, 'Failed to infer column type.'
                                     'If data is inaccessible, please turn off inference.')
    return dataflow
