...
class DatacacheStore(object):
    ...
    def _initialize(self, workspace, datacache_store_dto):
        ...
        if datacache_store_dto.cache_policy is not None:
            if datacache_store_dto.cache_policy.singularity_settings is not None:
                priority = None
                sla_tier = None
                if datacache_store_dto.cache_policy.singularity_settings.priority is not None:
                    priority = datacache_store_dto.cache_policy.singularity_settings.priority.value
                if datacache_store_dto.cache_policy.singularity_settings.sla_tier is not None:
                    sla_tier = datacache_store_dto.cache_policy.singularity_settings.sla_tier.value
                singularity_data_management_compute_target_info = OrderedDict([
                    ('name', datacache_store_dto.cache_policy.data_management_compute_name),
                    ('instance_type', datacache_store_dto.cache_policy.singularity_settings.instance_type),
                    ('priority', priority),
                    ('locations', datacache_store_dto.cache_policy.singularity_settings.locations),
                    ('sla_tier', sla_tier)
                ])
                singularity_data_management_compute_target_formatted_info = \
                    ','.join(['{}: {}'.format(k, v) for (k, v) in
                             singularity_data_management_compute_target_info.items()])
                self._data_management_compute_target = \
                    '({0})'.format(singularity_data_management_compute_target_formatted_info)
            else:
