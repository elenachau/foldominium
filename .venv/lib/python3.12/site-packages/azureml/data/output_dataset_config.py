...
class TransformationMixin:
    ...
    def read_delimited_files(
        ...
        """
        dprep = dataprep_stub()
        full_dataprep = dataprep()
        dataflow = dprep.DataflowStub()
        dataflow = TransformationMixin._filter_path(full_dataprep, dataflow, path_glob)
        dataflow = dataflow.parse_delimited(separator=separator, headers_mode=header, quoting=False)
        if partition_format:
            dataflow = dataflow._add_columns_from_partition_format(partition_format)
    ...
    def _filter_path(dprep, dataflow, path_glob):
        ...
        return dataflow.filter(
            dprep.RegEx(path_glob).is_match(dprep.api.functions.get_stream_name(dprep.col('Path')))
        )
